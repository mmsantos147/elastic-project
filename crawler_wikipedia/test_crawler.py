...
def build_reference_mapping(wiki_docs, wiki_titles, max_nodes=30):  # Padrão alterado para 30
    """
    ...
    # Limitar a 30 documentos para testes
    test_limit = min(30, len(wiki_docs))
    print(f"MODO DE TESTE: Limitando o processamento a {test_limit} documentos")
    
    doc_items = list(wiki_docs.items())[:test_limit]
    
    # Usar tqdm para mostrar barra de progresso
    for doc_id, doc in tqdm(doc_items, desc="Processando páginas"):
...
def main():
    ...
    # Construir mapeamento (limitado a 30 documentos para teste)
    reference_map = build_reference_mapping(wiki_docs, wiki_titles)
